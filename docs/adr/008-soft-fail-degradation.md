...
let loadError = "";
try {
  const { BamlRuntime, ClientRegistry } = await import("@boundaryml/baml");
} catch (err) {
  available = false;
  loadError = err.message;
}

pi.events.emit("pi-baml:ready", {
  available,
  createExecutor: available ? realImpl : () => { throw new Error(`pi-baml: BAML runtime unavailable: ${loadError}`); },
  // ...
});
```
